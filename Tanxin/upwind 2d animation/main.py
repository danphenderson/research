...
def solve_upwind_1D(a_func, u0_func, Nx=200, xmin=0.0, xmax=1.0, CFL=0.8, T_final=1.0, save_intervals=5):
    """
...
def solve_upwind_2d_3d_animation(a_func, b_func, u0_func,
                                 Nx=100, Ny=100,
                                 xmin=0.0, xmax=1.0,
                                 ymin=0.0, ymax=1.0,
                                 CFL=0.4, T_final=1.0,
    ...
    for n in range(1, num_steps + 1):
        ...
        for i in range(1, Nx):
            for j in range(1, Ny):
                ...
                dx_term = (u[i, j] - u[i - 1, j]) / dx if ax >= 0 else (u[i + 1, j] - u[i, j]) / dx
                dy_term = (u[i, j] - u[i, j - 1]) / dy if bx >= 0 else (u[i, j + 1] - u[i, j]) / dy
    ...
    ani = animation.FuncAnimation(fig, update_surface, frames=len(frames), interval=50, blit=False)
...
solve_upwind_2d_3d_animation(a_func=a_const,
                             b_func=b_const,
                             u0_func=u0_gaussian,
                             T_final=1.0)
