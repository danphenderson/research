...
\begin{document} \maketitle
    ...
    \section{Introduction}
        ...
        \subsection{Physiological Background}
            ...
            \begin{flalign*}
                    \quad & W_0 := D \cdot \sqrt{\frac{\omega \rho}{\eta}} &&
            \end{flalign*}
            a dimensionless parameter comparing the frequency $\omega$
            of the pulse wave to the blood's kinematic viscosity $\eta$ and the vessel diameter $D$.
            The Reynolds number $Re$ characterizes flow in blood vessels:
            \begin{flalign*}
                    \quad & Re := \frac{\rho U D}{\eta}, &&
            \end{flalign*}
            where $U$ is the mean flow velocity. Low $Re$ indicates laminar flow,
            while high $Re$ suggests turbulent flow. \todo{replace D with radius R}
            \todo{Be more explicit that we are deteriving a 1D "lumped model"?}
            ...
            \subsubsection*{Properties of Blood}
                ...
                At \textit{microscopic scales}, our models will break down—fluids as fluids
                are discrete collections of molecules, not continuous fields. However, at the
                macroscopic scale we can hope that such models remain accurate.
                The Incompressibility assumption is reasonable for blood, as water comprises
                $\approx 55\% \cdot .90 \approx 50\%$. Despite blood being a heterogeneous mixture of
                compounds, we assume it is a single constituent, homogeneous and isotropic Newtonian fluid.

                \begin{definition}
                    \label{def:newt-fluid}
                    Let $V$ be a fluid volume $V$ with dynamic viscosity $\eta = \frac{\tau}{\dot{\gamma}} \equiv$ const.,
                ...
                \begin{remark}[Dynamic vs. Kinematic Viscosity]
                    Let $\eta \in \R^+$ denote the \emph{dynamic viscosity} of blood, and let $\mu \in \R^+$ denote the \emph{kinematic viscosity}. Then:
                    \begin{equation}
                    \mu := \frac{\eta}{\rho}
                    \end{equation}
                    ...
                    Both quantities appear in our hemodynamic models.
                \end{remark}
            \medskip
        \medskip
        ...
        \subsection{Navier Stokes System}
            ...
            \end{flalign*}
            To simulate the kinematics of blood flow on the time interval $[0, T]$
            in the lumen $\Omega(t) \subset \R^3$ of a blood vessel,
            we use the Generalized Newtonian Navier-Stokes (NS) system:
            \begin{definition}[Incompressible Viscous NS (Conservative Form)]
                ...
                \begin{flalign*}
                    ...
                    \end{cases*} &&
                \end{flalign*}
                where $\vct{f}$ is an external force field (e.g., gravity), $\mu|\vct{D}|^2$ is the dynamic viscosity and $\vct{D}$ is the Cauchy deformation tensor defined as
                \begin{flalign*}
                    \quad & \vct{D}(\vct{u}) := \tfrac12\big(\nabla\vct{u} + \nabla\vct{u}^{\mathsf T}\big). &&
                \end{flalign*}
            \end{definition}

            \medskip
            
            We rewrite Eq.~\ref{def:NSEquations} in standard form (as found in \cite{ns-formulation}, \cite{MathematicalEngineering}.)
            \begin{flalign*}
                \quad &\rho\big(\partial_t \vct{u} + (\vct{u}\cdot\nabla)\vct{u}\big) = -\,\nabla p \;+\; \Divbf\!\Big( 2\,\mu|\vct{D}|^2\,\vct{D}\Big) \;+\; \rho\,\vct{f}&&\\
                ...
                \quad &\iff F(\dots) \;=\; \vct{f}&&  
            \end{flalign*}
            
            \medskip

            \begin{remark}[Interpretation of NS Terms]
                The first equation of \ref{def:NSEquations} is derived from
                Newton's second law of motion. The term $(\vct{u} \cdot \nabla) \vct{u}$ represents the convective acceleration,
                $\frac{\nabla p}{\rho}$ is the pressure gradient force per unit mass, and
                $\Divbf\big(\frac{2}{\rho} \mu |\vct{D}|^2 \vct{D}\big)$ governs the
                viscoelastic and diffusive features of the flow.
                The second equation of \ref{def:NSEquations} follows from the \emph{incompressibility condition},
                implying that the fluid density remains constant along flow lines.
            \end{remark}

            \medskip

            \begin{remark}[Fluids Model]\
                The fluid dynamics model is given by the Cauchy stress tensor:
                \begin{flalign*}
                    \quad \vct{T} = -p \vct{I} + \vct{S} \quad : \quad \begin{cases*}
                        ~p \text{ is the hydrodynamical pressure},\\
                        ~p\vct{I} \text{ is the mean normal stress, } \\
                        ~\vct{S} \text{ reflects rheological behavior}
                    \end{cases*}&&
                \end{flalign*}
                Under the Newtonian blood assumption $\vct{S} = 2\mu \vct{D}$.
            \end{remark}
            ...
            \subsubsection{Converting to Cylindrical Coordinates}
    ...
    \mapsto (x, y, z)$. For simplicity, we assume 
    ...
    \end{figure}
        ...
        which an arterial wall is subjected to due to the blood movement.
            ...
            \subsubsection{Dimension reduced Models}
                \label{subsubsec:dim-reduced-models}
