...
\begin{document} \maketitle
    ...
    \section{Introduction}
        ...
        \subsection{   Physiological Background}
            ...
            \subsection{Fluid Model of Blood}
                ...
                We define the blood's velocity, thermodynamic pressure, and density fields as
                \begin{flalign*}
                    \quad & \vct{u}: \Omega(t) \to \R^3, ~ (x, y, z) \mapsto (u_1(x, y, z), ~ u_2(x, y, z), ~u_3(x, y, z)), && \\
                    \quad & p: \Omega(t) \to \R, ~ (x, y, z) \mapsto p(x, y, z), && \\
                    \quad & \rho: \Omega(t) \to \R^+, ~ (x, y, z) \mapsto \rho(x, y, z). &&
                \end{flalign*}
        ...
        \subsection{Navier Stokes System}
            ...
            and very large shear rates, resp., and $\kappa > 0$ and $n \in (-0.5, 0)$ are model parameters.
            ...
            \subsubsection{NS in Cylindrical Coordinates}
                ...
                \begin{tikzpicture}[scale=1, every node/.style={font=\small},>=Stealth]
                    \definecolor{arteryred}{RGB}{196,22,34}
                    \definecolor{vesselgray}{RGB}{220,220,220}

                    % Curved vessel at the top
                    \begin{scope}[shift={(0,3.1)}]
                        \filldraw[line width=1pt, draw=black, fill=vesselgray]
                            (-3.2,0.35)
                            .. controls (-1.8,1.4) and (1.5,0.8) .. (3.2,1.1)
                            .. controls (3.5,0.4) and (3.3,-0.4) .. (3.1,-1.1)
                            .. controls (1.0,-1.2) and (-1.4,-1.5) .. (-3.2,-0.7)
                            .. controls (-3.5,-0.1) and (-3.5,0.1) .. cycle;

                        \draw[dashed, line width=0.8pt]
                            (-3.1,-0.2)
                            .. controls (-1.6,0.6) and (1.8,0.2) .. (3.1,0.4);

                        \begin{scope}[shift={(-3.2,-0.1)}, rotate=18]
                            \shade[ball color=arteryred] (0,0) ellipse (0.55 and 0.25);
                            \draw[line width=0.9pt, color=arteryred!80!black]
                                (0,0) ellipse (0.55 and 0.25);
                        \end{scope}

                        \begin{scope}[shift={(3.15,0.25)}, rotate=-12]
                            \shade[ball color=arteryred] (0,0) ellipse (0.52 and 0.23);
                            \draw[line width=0.9pt, color=arteryred!80!black]
                                (0,0) ellipse (0.52 and 0.23);
                        \end{scope}

                        \node[anchor=west, font=\bfseries] at (-3.4,1.4)
                            {Curved vessel};
                    \end{scope}

                    % Transformation arrow
                    \draw[very thick, -{Stealth[length=8pt]}] (0,1.6) -- (0,0.2);
                    \node[font=\bfseries] at (0.7,0.9) {Transformation};

                    % Straightened cylindrical vessel
                    \begin{scope}[shift={(0,-0.9)}]
                        \def\height{0.48}
                        \def\length{3.6}
                        \def\offset{1.8}

                        \path[fill=vesselgray, draw=black, line width=0.9pt]
                            (-\offset,\height) .. controls (-0.9,0.95) and (0.9,0.95) .. (\offset,\height)
                            -- (\offset,-\height) .. controls (0.9,-0.95) and (-0.9,-0.95) .. (-\offset,-\height)
                            -- cycle;

                        % Cross-sectional slices
                        \foreach \z in {-\offset,-0.2,\offset} {
                            \begin{scope}[shift={(\z,0)}]
                                \shade[ball color=arteryred] (0,0) ellipse (0.32 and 0.13);
                                \draw[line width=0.8pt, color=arteryred!80!black]
                                    (0,0) ellipse (0.32 and 0.13);
                            \end{scope}
                        }

                        % Surface label
                        \node[above right] at (0.85,\height+0.05) {$S(z,t)$};

                        % Radius arrow
                        \draw[->, line width=0.8pt] (-0.2,0) -- (-0.2,\height)
                            node[pos=1, above left] {$r = R(z,t)$};

                        % Domain label
                        \node[below left] at (-\offset, -\height-0.3) {$\Omega(t)$};

                        % z = l label
                        \node[below] at (\offset,-\height-0.3) {$z = l$};

                        % Coordinate axes
                        \draw[->, line width=0.9pt]
                            (-\offset-0.9,-\height-0.45) -- (-\offset-0.9,\height+0.7)
                            node[above] {$x$};
                        \draw[->, line width=0.9pt]
                            (-\offset-0.9,-\height-0.45) -- (-\offset-1.9,-\height-0.45)
                            node[left] {$y$};
                        \draw[->, line width=0.9pt]
                            (-\offset-0.9,-\height-0.45) -- (\offset+0.6,-\height-0.45)
                            node[right] {$z$};
                    \end{scope}
                \end{tikzpicture}
                \caption{Schematic transformation from a curved vessel to a straightened cylindrical domain.}
                \label{fig:curved-to-straight}
            \end{figure}
    ...
    \end{figure}
        \todo{Improve readability of schematic and perform a change of basis}
    ...
    \section{Mathematical Models of Blood Flow}
        ...
        computational resources, and desired accuracy. Our aim is to determine
        the velocity and pressure fields of blood flow in various vascular geometries,
        ranging from simple straight vessels to complex arterial networks. By determining
        these fields evolution over time, we can approximate the hemodynamic forces acting
        on the vessel walls.
        ...
        \subsection{Dimension-Reduced Models of Blood Flow}
            ...
            simplifies the 3D Navier-Stokes equations by averaging over the cross-sectional area of the vessel,
