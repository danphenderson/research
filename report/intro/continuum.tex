...
Our aim is to simulate the hemodynamics of a time-dependent spatial fluid domain $\Omega(t)\subset\R^N$, where
\begin{flalign*}
...
\end{flalign*}
The spatial-temporal domain is
\begin{flalign*}
    \quad & \Omega \;:=\; \{ (x, t) \in \R^{N+1} ~:~ x \in \Omega(t), ~ t \in I_T \}, &&
\end{flalign*}
which is r.t. as the computational domain.
On $\Omega(t)$, we say that
\begin{flalign*}
...
\end{flalign*}
A second-order (rank-2) tensor $\vct T$ defines a linear map from $\R^N \to \R^N$, and once a basis is fixed,
$\exists ~ A \in \R^{N\times N}$ s.t. $\vct T(\vct x) = A \,\vct x$ for all $\vct x \in \Omega(t)$.
\paragraph{Modeling Framework}
At microscopic scales the continuum hypothesis breaks down as matter is a discrete collection of molecules,
but at macroscopic scales empirical evidence suggests our models are accurate. Let the blood's velocity, thermodynamic pressure, and density fields be
\begin{flalign*}
...
\end{flalign*}
Our assumption that blood is a simple fluid ensures $\vu, ~p, \text{ and } \rho$ is sufficiently smooth in $\Omega$.
\begin{remark}[Simple Fluid]
    A \emph{simple fluid} is a fluid where the Cauchy stress tensor depends only on the current state of deformation and rate of deformation
    at each point in the fluid, and not on the history of deformation.
    The contrary are \emph{complex fluids}, where stress may depend on past deformations
    r.t.a. viscoelastic fluids, which exhibit both viscous and elastic characteristics.
    Here, we assume blood is a simple fluid, one that isn't subject to memory effects.
\end{remark}
\begin{definition}
...
positioned at $\vx \in \Omega(t)$ and moving with velocity $\vu(x, t)$.
By the incompressibile assumption of blood, the material density of $V(t)_{\vx}$ is $\rho(\vx, t) \equiv \rho_0$
for all $\vx \in \Omega(t)$ and $t \in I_T$. Implying $D_t \rho = 0$, so mass conservation in $\Omega(t)$ reads
\begin{flalign*}
...
\begin{flalign*}
    \quad \rho(\partial_t \vu + (\vu\cdot\nabla)\vu ) \;=\; \Divbf(\vT) + \rho \vfb, \qquad \text{in} ~ \Omega(t) &&
\end{flalign*}
where $\vT$ is the Cauchy stress tensor, $\vfb$ may be some body force per unit mass $\big[\dfrac{m}{s^2}\big]$.
...
\end{flalign*}
Note that $\vu \mapsto \vct{D}(\vu)$ captures spatial deformations of element $V_{\vx}$ in $\Omega$ under flow $\vu$.
\begin{definition}
...
\end{flalign*}
One may make a distinction between incompressible fluids and incompressible flows, and by
assuming the former w.a.t.s. the divergence-free (senodial) condition on velocity
feild $\vu$.
\begin{definition}[Incompressibile fluid]
    \label{def:incompressible-fluid}
    An element $V \in \Omega$ with constant density $\rho(\vx, t)$ for all $\vx \in V(t), t \in I_T$
    is an \emph{incompressible fluid}.
\end{definition}
\begin{definition}[Incompressible flow]
\label{def:incompressible-flow}
    An element $V \in \Omega$ subject to $\vu$ s.t. $V$ with constant rate of material density change
    (in both space and time so that $D_t \rho = 0$ for all $t \in I_T$) undergoes an \emph{incompressible flow} in $\Omega$.
\end{definition}
For all material elements $V \in \Omega$, \ref{def:incompressible-fluid} implies \ref{def:incompressible-flow}.
Note, the converse is not generally true: an incompressible flow ($D_t\rho=0$) only preserves density
along particle paths and allows $\rho=\rho(\vx)$ to vary spatially; but when the initial density
is distributed uniformly in space, incompressible flow implies incompressible fluid.
