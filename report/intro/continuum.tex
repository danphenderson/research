...
\begin{definition}
    ...
    Let $\phi \in C^k(\Omega_B)$ and define
    \[
        \phihat(t,\vxi) \defined \phi\bigl(t, \vx(t,\vxi)\bigr).
    \]
    The \emph{material derivative} of $\phi$ is
    \[
        D_t \phi(t,\vx(t,\vxi))
        \defined \pd_t \phihat(t,\vxi)
        = \frac{d}{dt}\,\phi\bigl(t,\vx(t,\vxi)\bigr),
        \qquad \vxi \in \Omega_B(0).
    \]
    Equivalently, for $(t,\vx)\in I_T\times\Omega_B(t)$,
    \[
        D_t \phi(t,\vx)
        \defined \pd_t \phihat\bigl(t,\La_t^{-1}(\vx)\bigr).
    \]
\end{definition}
...
\begin{theorem}[Reynolds Transport Theorem]
    ...
    then for $\phi \in C^1(\OmgB)$ we have
    \begin{flalign*}
        \quad \dfrac{d}{dt} \int_{V(t)} \phi(t, \vx) \, d\vx
            & \equals \int_{V(t)} D_t \phi(t, \vx) \, d\vx &&\\
            & \equals \int_{V(t)} \pd_t \phi(t, \vx) \, d\vx + \int_{\p V(t)} \phi(t, \vx) \, \inp{\vu(t, \vx)}{\nhat} \, dS, &&
    \end{flalign*}
    where $\nhat$ is the outward unit normal on $\p V(t)$.
\end{theorem}
\begin{proof}
