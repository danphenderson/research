...
The following lemma shows that $\partial_t J$ relates to the fluids divergence $\Div(\vu)$.

\medskip

\begin{lemma}
    \label{lem:jacobian-determinant-derivative}
    The Jacobian determinant $J(t)(\vx)$ satisfies
    \begin{flalign*}
        \quad & D_t J(t)(\vx) \;=\; J(t)(\vx) \, \Div(\vu(t, \vx)) \quad \forallin{(t, \vx)}{\OmgB}. &&
    \end{flalign*}
    \begin{proof}
        Ref~\cite{book2}, lemma 2.1, pg. 20.
    \end{proof}
\end{lemma}
...
\end{definition}
I.m.b.s. for all material elements $V \in \OmgB$, \ref{def:incompressible-fluid} implies \ref{def:incompressible-flow}.
...
for all $\vx \in \Omgt$ and $t \in I_T$. Constant blood density implies $D_t \rho = 0$, i.e., incompressibile flow in $\OmgBt$
\begin{flalign*}
