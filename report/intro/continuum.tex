W.a.t. simulate blood flow in a time-dependent fluid domain $\Omega_B(t) \subset \R^N$,
where typically $N = 3$. For each $t \in I_T := [0, T] \subset \R$, with $T > 0$, we define
\begin{flalign*}
    \quad \Omega_B(t) &:= \{\vx \in \R^N : \vx \text{ lies inside the vessel at time } t\}. &&
\end{flalign*}
Formally, we model the fluid region as a time-dependent family of open sets
$\{\Omega_B(t)\}_{t \in I_T} \subset \R^N$ occupied by blood at time $t$.
...
are denoted $\{\vct e_1, \vct e_2, \vct e_3 \}$ and we let $\{\ihat, \jhat, \khat\}$ r.t.
