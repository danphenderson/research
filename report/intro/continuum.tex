Our aim is to simulate bloods flow in a time-dependent fluid domain $\OmgBt \subset\R^N$, where
\begin{flalign*}
    \quad \Omg(t) &:= \{ x \in \R^N ~:~ x \text{ lies inside the blood vessel at time } t \} &&
\end{flalign*}
...
\end{flalign*}
We r.t. $\OmgBar$ as the computational domain. On the spatial fluid and vessel domain $\Omg(t)$, we let
\begin{flalign*}
...
\begin{flalign*}
    \quad & \vu: \Omg(t) \to \R^3, ~ (x, y, z) \mapsto (u_1(x, y, z), ~ u_2(x, y, z), ~u_3(x, y, z))^\top, \quad \bigg[\; \frac{m}{s^2} \;\bigg] && \\
    \quad & p: \Omg(t) \to \R, ~ (x, y, z) \mapsto p(x, y, z), \quad \bigg[\; Pa \equiv \frac{N}{m^2}\;\bigg] && \\
    \quad & \rho: \Omg(t) \to \R^+, ~ (x, y, z) \mapsto \rho(x, y, z).\quad \bigg[\; \frac{kg}{m^3} \;\bigg]&&
\end{flalign*}
Our assumption that blood is a simple fluid ensures $\vu, ~p, \text{ and } \rho$ is sufficiently smooth in $\Omg$.
...
\label{def:incompressible-flow}
    An element $V \in \Omg$ subject to $\vu$ s.t. $V$ with constant rate of material density change
    (in both space and time so that $D_t \rho = 0$ for all $t \in I_T$) undergoes an \emph{incompressible flow} in $\Omg$.
\end{definition}
For all material elements $V \in \Omg$, \ref{def:incompressible-fluid} implies \ref{def:incompressible-flow}.
