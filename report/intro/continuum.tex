...
Our aim is to simulate the hemodynamics of a time-dependent fluid domain $\Omega(t)\subset\R^N$, where
\begin{flalign*}
...
\end{flalign*}
occasionally r.t. as the computational domain.
On the spatial domain $\Omega(t)$, we let
\begin{flalign*}
    \quad & \phi:\Omega(t)\to\R \;\text{ r.t. a \emph{scalar field}}, & \\
    \quad & \vct f:\Omega(t)\to\R^N \;\text{ r.t. a \emph{vector field}}, &&\\
    \quad & \vct T:\Omega(t)\to\R^{N\times N} \;\text{ r.t. a \emph{(second-order) tensor field}}. &&
\end{flalign*}
