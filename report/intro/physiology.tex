...
to the viscous effects determined by the kinematic viscosity $\mu \defined \eta / \rho ~ \units{\frac{cm^2}{s}}$:
\begin{flalign*}
        \quad W_0 & \defined \sqrt{\frac{\rho \omega U}{\eta U L^-2}} \equals \sqrt{L^2 \cdot \frac{\omega}{\mu}}
        \units{-} \quad \text{ s.t.} \quad \begin{cases*}
                ~ L: \text{ characteristic length scale } \units{cm} \\
                ~ U: \text{ characteristic velocity scale } \units{\frac{cm}{s}} \\
                ~ \eta: \text{ dynamic viscosity } \units{Pa \cdot s}\\
                ~ \rho: \text{ blood density } \units{\frac{kg}{cm^3}} \\
        \end{cases*}&&
\end{flalign*}
High $W_0$ indicates large, rapid pulses where inertial effects dominate viscous effects
while low $W_0$ indicates small, slow pulses where viscous effects dominate inertial effects.
The Reynolds number $Re$ characterizes flow in blood vessel
\begin{flalign*}
        \quad & Re := L \cdot \frac{\rho U}{\eta} \units{-}.&&
\end{flalign*}
...
and laminar flow in small vessels. Note, most often we set $L = 2R$ where $R$ is
...
the coronary artery. \blue{leading to reduced and turbulent blood flow. (Add citations
