...
\begin{flalign*}
    \quad \norm{\phi}_{C^k(K)} &\defined \sum_{|\alpha| \leq k} \sup_{\vx \in K}
    \left| D^\alpha \phi(\vx) \right|. &&
\end{flalign*}
meaning that every Cauchy sequence $\{ \phi_j \}_{\forall j \in \N} \subset C^k(K)$ converges
to a limit in $C^k(K)$. Thus, by definition the complete normed sp. $(C^k(K), \, \norm{\cdot}_{C^k(K)})$ is banach;
in particular $C(\OmgBar)$ is banach with norm
\begin{flalign*}
...
\td{
    TODO: forgoe unless working with a Lipschitz boundary, for now we'll assume $\pOmg$ is $C^1$.\\
    ...
    just cite.\\
    ~- Proceed into Hölder spaces, then Hilbert and Sobolev spaces.\\
}
\medskip
