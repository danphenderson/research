...
Given domain $\Omega$, the function space $\mathcal{F}(\Omg)$ is the vec.
sp. $(F(\Omg), \, K)$ of scalar valued functions $f:\Omg \to K$. E.g. $C^0(\Omg)$ r.t.
...
% TODO: more on sp.s: topological to metric to normed to inner-product sp.,
%   then completeness and banach sp. discussion; Hilbert sp.s are banach.
\medskip

% basic feild notation
Let $n = \dim(\Omega)$, then we say
\begin{flalign*}
    \quad & \phi:\Omg\to\R \;\text{ r.t. a \emph{scalar field}}, & \\
    \quad & \vct f:\Omg\to\R^{n} \;\text{ r.t. a \emph{vector field}}, &&\\
    \quad & \vct T:\Omg\to\R^{n\times n} \;\text{ r.t. a \emph{(second-order) tensor field}},&&
\end{flalign*}
and we say that $\phi(\vx)$, $\vct f(\vx)$, and $\vct T(\vx)$ are the values of the fields
at point $\vx \in \Omg$. \footnote{The tensor $\vct T\vx$ is a linear map from $\R^n \to \R^n$,
and once a basis is fixed, $\exists ~ A \in \R^{n\times n}$ s.t. $\vct T(\vct x) = A \,\vct x$ for all $\vct x \in \Omg$.}
...
\end{flalign*}
If $\pOmg \in C^1$, then $f \in L^{1}(\OmgBar)$ and the integral at the boundary is
\begin{flalign*}
...
\begin{flalign*}
    \quad \Divbf \, \vT \defined \left( \Div \, \vct T_{i\cdot} \right)_{i\in[N]}, &&
\end{flalign*}
