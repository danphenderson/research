...
If $\phi \in C^1(\Omg)$, the partial derivative w.r.t. coordinate
$x_i$ is denoted
\begin{flalign*}
    \quad \pd_{x_i} \phi = \frac{\pd \phi}{\pd x_i}, &&
\end{flalign*}
the gradient of $\phi$ is $\nabla \phi = \left(\pd_{x_i} \phi\right)_{i\in[n]}$ and
the Laplacian of $\phi$ is $\del \phi = \sum_{i\in[n]} \pd_{x_i}^2 \phi.$
\footnote{In coordinate free-terms the gradient $\nabla \phi(\vx)$ is defined s.t.
$d\phi = \inp{\nabla \phi}{\dx}$ for all infinitesimal displacements $\dx$.}
...
\end{flalign*}
If $\pOmg \in C^1$, then $\phi \in C^{1}(\OmgBar) \supset L^1(\pOmg)$ and the integral at the boundary is
\begin{flalign*}
...
For vector valued $\vf \in C^1(\Omg; \R^N)$ s.t. $\vf = (f_1, \, \ldots, f_N)^\top$, the divergence operator is
\begin{flalign*}
...
For tensor valued $\vT \in C^1(\Omg; \R^{N\times N})$ s.t. $\vT = (T_{ij})_{i,j\in[N]}$, the divergence
operator is
\begin{flalign*}
